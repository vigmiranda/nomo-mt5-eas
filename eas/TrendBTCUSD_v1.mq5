//+------------------------------------------------------------------+
//| TrendBTCUSD_v1.mq5                                               |
//| Nomo - impulso BTC + soft lock + trailing                        |
//| Spread tipico ~2900 pts (~US$29 em ~77000) | grafico H1          |
//| v1.10: soft lock acima do spread antes do trailing               |
//+------------------------------------------------------------------+
#property copyright "Vitor"
#property version   "1.10"
#property strict

input double InpRiskPercent      = 0.25;  // Risco por trade (% saldo) - crypto agregada
input double InpMaxLots          = 0.50;  // Teto de lote
input bool   InpUseFixedLots     = false;
input double InpFixedLots        = 0.01;

input int    InpBreakBars        = 10;    // Rompe max/min (H1: mais confirmacao)
input int    InpMaPeriod         = 20;    // EMA (filtro leve)
input bool   InpUseEmaFilter     = true;
input double InpMinBodyATR       = 0.55;  // Corpo minimo = fator * ATR
input int    InpATRPeriod        = 14;

// Calibrado BTCUSD Nomo (spread ~2900 pts observado)
// SoftLockStart > spread: so trava apos lucro cobrir o spread (~2900)
input int    InpStopPoints       = 8000;  // SL ~US$80
input int    InpTakePoints       = 16000; // TP ~US$160
input int    InpTrailStartPoints = 5800;  // Trailing apos ~US$58 de lucro
input int    InpTrailStepPoints  = 1500;  // Degrau trailing ~US$15
input int    InpLockPoints       = 2500;  // SL travado ~US$25 atras
input int    InpSoftLockStart    = 3800;  // Trava apos lucro > spread (2900)
input int    InpSoftLockPts      = 1200;  // SL = entrada + ~US$12

input int    InpMaxSpreadPoints  = 3200;  // Spread max (normal ~2900)
...
int OnInit()
{
   ...
   Print("TrendBTCUSD_v1.10 | ", _Symbol, " ", EnumToString(_Period));
   Print("maxPos=", InpMaxOpenPositions, " | digits=", SymbolInfoInteger(_Symbol, SYMBOL_DIGITS),
         " point=", DoubleToString(point, 8));
   Print("spread=", spr, " pts (= ", DoubleToString(spr * point, 2),
         " US$) | max=", InpMaxSpreadPoints);
   Print("stop=", InpStopPoints, " | take=", InpTakePoints,
         " | softLock=", InpSoftLockStart, "+", InpSoftLockPts,
         " | trail=", InpTrailStartPoints);

   if(spr > 0 && InpSoftLockStart <= spr)
      Print("AVISO: SoftLockStart (", InpSoftLockStart, ") <= spread (", spr,
            "). Aumente para trava apos cobrir spread.");
...
void OnTick()
{
   ...
   if(spread > InpMaxSpreadPoints)
   {
      if(InpVerboseLog)
         Print("Skip: spread alto ", spread, " > ", InpMaxSpreadPoints,
               " (~US$", DoubleToString(spread * point, 2), ")");
      return;
   }
   ...
   {
      if(InpVerboseLog)
         Print("Skip: stop ", InpStopPoints, " < ", minStop, " (1.5x spread)");
      return;
   }
...
void ManageTrailing()
{
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      else if(favorMove >= InpTrailStartPoints)
         Print("TRAIL ticket=", ticket, " favor=", (int)favorMove, "pts SL ",
               DoubleToString(sl, digits), "->", DoubleToString(newSL, digits));
      else
         Print("SOFT ticket=", ticket, " favor=", (int)favorMove, "pts SL ",
               DoubleToString(sl, digits), "->", DoubleToString(newSL, digits));
   }
}
...
bool OpenTrade(ENUM_ORDER_TYPE type)
{
   ...
   request.deviation = 80;
   request.magic = InpMagic;
   request.comment = "TrendBTCUSD_v1.10";
   request.type_filling = ResolveFilling();
